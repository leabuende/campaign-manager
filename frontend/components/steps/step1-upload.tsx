...
export function Step1Upload({ onNext, campaignId }: Step1UploadProps) {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <ProgressStep stepNumber={1} title="Campaign Brief" description="Upload your assets and brief" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-4">Drag and drop your PDF brief here or click to browse</p>
          {briefFile && (
            <div className="bg-accent/10 rounded-lg p-3 mb-4">
              <p className="text-sm font-medium text-accent">{briefFile.name}</p>
            </div>
          )}
          <input type="file" accept=".pdf" onChange={handleBriefClick} className="hidden" id="brief-input" />
      ...
      <div className="mt-8 flex justify-end">
        <Button
          onClick={() => onNext({ briefFile, productImages, campaignId })}
          disabled={!briefFile || productImages.length === 0}
